...
function down(e){DIR.drag=true;DIR.px=(e.touches?e.touches[0].clientX:e.clientX);DIR.py=(e.touches?e.touches[0].clientY:e.clientY);}
function move(e){
  if(!DIR.drag)return;
  const x=(e.touches?e.touches[0].clientX:e.clientX), y=(e.touches?e.touches[0].clientY:e.clientY);
  DIR.orbA-=(x-DIR.px)*0.006; DIR.orbH=clamp(DIR.orbH+(y-DIR.py)*0.004,0.05,1.45);
  DIR.px=x;DIR.py=y;
  if(!DIR.manual){DIR.manual=true;$('btnCam').classList.remove('on');$('btnCam').textContent='Free Cam';}
  e.preventDefault();
}
function up(){DIR.drag=false;}
el.addEventListener('mousedown',down); addEventListener('mousemove',move); addEventListener('mouseup',up);
el.addEventListener('touchstart',down,{passive:true}); el.addEventListener('touchmove',move,{passive:false}); addEventListener('touchend',up);
