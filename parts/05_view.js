...
function audioResume(){
  ...
  audioPrime();
  if(!audioGestureAt) audioGestureAt=Date.now();
  audioBadge();
  decodeAssets();
...
function audioBadge(){
  ...
  if(AC&&AC.state==='running'){ el.classList.remove('on'); return; }
  /* Never nag before the browser has had a fair chance. iOS routinely passes
     through 'suspended' and 'interrupted' on its way to running, and on a
     phone the badge was appearing during that transition — a banner that
     flashes up every time is worse than no banner. It now needs a real
     gesture to have happened and the context to still be stuck a couple of
     seconds later, and it keeps quietly retrying in the meantime, which is
     what actually recovers an interrupted context after a call or a lock. */
  const stuck = audioGestureAt && (Date.now()-audioGestureAt > 2500);
  if(AC && stuck){ const p=AC.resume(); if(p&&p.catch) p.catch(()=>{}); }
  el.classList.toggle('on', !!AC && !!stuck && (soundOn||MUS.on));
}
